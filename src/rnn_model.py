...
class BHW2RNNDecoder(nn.Module):
    ...
    def forward(self, encoder_hidden, target_idx, teacher_forcing_ratio=0.5):
        decoder_hidden = encoder_hidden
        targets = self.embeddings(target_idx)
        outputs = []
        decoder_input = targets[:, 0:1, :]
        
        for t in range(target_idx.shape[1]):
            decoder_output, decoder_hidden = self.rnn(decoder_input, decoder_hidden)
            token_logits = self.linear_head(decoder_output)
            outputs.append(token_logits)
            
            if torch.rand(1).item() < teacher_forcing_ratio:
                decoder_input = targets[:, t+1:t+2, :]
            else:
                decoder_input = self.embeddings(token_logits.argmax(dim=-1))
        
        return torch.cat(outputs, dim=1), decoder_hidden
